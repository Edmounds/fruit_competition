...
int main(int argc, char *argv[])
{
    ...
    // 从命令行参数中读取目标位姿
    // 参数格式: cartesian_controller target_x target_y target_z
    double target_x = 0.0;
    double target_y = 0.0;
    double target_z = -0.2;
    ...
    if (argc >= 4)
    {
        ...
        {
            RCLCPP_ERROR(rclcpp::get_logger("cartesian_controller"),
                         "解析命令行参数失败: %s，使用默认值", e.what());
        }
    }
    else
    {
        RCLCPP_WARN(rclcpp::get_logger("cartesian_controller"),
                    "未提供足够的命令行参数，使用默认值: x=%.2f, y=%.2f, z=%.2f",
                    target_x, target_y, target_z);
    }

    // 创建一个日志记录器
    auto const logger = rclcpp::get_logger("cartesian_controller");
