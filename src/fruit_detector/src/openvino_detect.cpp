...
void Inference::InitializeModel(const std::string &model_path) {
    try {
        ...
        // 验证模型和推理请求的有效性
        if (!compiled_model_) {
            throw std::runtime_error("编译模型失败：compiled_model为空");
        }
        if (!inference_request_) {
            throw std::runtime_error("创建推理请求失败：inference_request为空");
        }
...
void Inference::RunInference(cv::Mat &frame) {
    ...
    if (!is_initialized_) {
        throw std::runtime_error("模型未正确初始化，无法进行推理");
    }
    ...
    try {
        ...
        throw std::runtime_error("OpenVINO推理失败: " + std::string(e.what()));
    }
    catch (const std::exception& e) {
        throw std::runtime_error("推理过程异常: " + std::string(e.what()));
    }
}
...
void Inference::Preprocessing(const cv::Mat &frame) {
    ...
    // 使用成员变量存储resized_frame，确保在推理期间数据有效
    resized_frame_.release();  // 释放旧数据
    cv::resize(frame, resized_frame_, model_input_shape_, 0, 0, cv::INTER_LINEAR);
    
    if (resized_frame_.empty()) {
        throw std::runtime_error("图像缩放失败");
    }
    ...
    const ov::Tensor input_tensor = ov::Tensor(
        ...
        resized_frame_.data                          // 使用持久化的图像数据指针
    );
...
void Inference::PostProcessing(cv::Mat &frame) {
    ...
    if (!output_tensor) {
        throw std::runtime_error("无法获取输出张量");
    }
    
    const float* detections = output_tensor.data<const float>();
    if (detections == nullptr) {
        throw std::runtime_error("输出张量数据为空");
    }
    ...
    for (int i = 0; i < detection_outputs.cols; ++i) {
        ...
        if (detection_outputs.rows < 4 + num_classes) {
            continue;  // 跳过无效的检测结果
        }
        
        // 提取类别概率
        cv::Mat classes_scores = detection_outputs.col(i).rowRange(4, 4 + num_classes);
