    ...
    : node_(node), imu_msgs(std::make_unique<sensor_msgs::msg::Imu>())
{
    ...
    // 创建IO上下文
    io_context_ = std::make_unique<drivers::common::IoContext>(2);
    serial_imu = std::make_unique<drivers::serial_driver::SerialDriver>(*io_context_);
...
DmImu::~DmImu()
{
    ...
    if (serial_imu && serial_imu->port()->is_open()) {
        serial_imu->port()->close();
    }
}
...
{
    try {
        serial_imu->init_port(imu_serial_port, *serial_config_);
        serial_imu->port()->open();
...
void DmImu::get_imu_data_thread()
{
    ...
    while (rclcpp::ok() && !stop_thread_)
    {
        ...
        try {
            ...
            size_t bytes_read = serial_imu->port()->receive(frame_header);
            if (bytes_read == 4) {
                memcpy(&receive_data.FrameHeader1, frame_header.data(), 4);
            } else {
                continue;
            }
            ...
            std::vector<uint8_t> remaining_data(57-4);
            size_t remaining_bytes = serial_imu->port()->receive(remaining_data);
            if (remaining_bytes == 57-4) {
                memcpy(&receive_data.accx_u32, remaining_data.data(), 57-4);
            } else {
                continue;
            } 
...
void DmImu::enter_setting_mode()
{
    ...
    try {
        for(int i=0;i<5;i++){
          if (serial_imu && serial_imu->port()->is_open()) {
              std::vector<uint8_t> data(txbuf, txbuf + sizeof(txbuf));
              serial_imu->port()->send(data);
          }
